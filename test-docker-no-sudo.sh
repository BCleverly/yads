...
# Docker Test Script for YADS No-Sudo Commands
# Run this inside the Docker container to test all commands without sudo
...
# Test YADS installation
test_yads_installation() {
    info "🧪 Testing YADS Installation"
    echo "============================="
    
    local tests_passed=0
    local tests_total=0
    
    # Test if yads command is available
    ((tests_total++))
    if test_command "command -v yads" "yads command availability"; then
        ((tests_passed++))
    fi
    
    # Test if yads help works
    ((tests_total++))
    if test_command "yads help" "yads help command"; then
        ((tests_passed++))
    fi
    
    # Test if yads version works
    ((tests_total++))
    if test_command "yads version" "yads version command"; then
        ((tests_passed++))
    fi
    
    # Test if yads status works
    ((tests_total++))
    ...
    echo
    info "YADS Installation: $tests_passed/$tests_total passed"
    echo
}
...
# Test YADS configuration commands (without sudo)
test_yads_configuration() {
    info "🧪 Testing YADS Configuration Commands (No Sudo)"
    echo "================================================="
...
main() {
    ...
    log "${CYAN}🧪 YADS Docker No-Sudo Test Suite${NC}"
    log "${BLUE}====================================${NC}"
    echo
    ...
    if ! command -v yads >/dev/null 2>&1; then
        error "yads command not found. YADS may not be installed properly"
        exit 1
    fi
    
    info "Starting comprehensive no-sudo tests in Docker..."
    echo
